                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>السجل التجاري</td>
                                        <td>{{ $info->commercial_register ?? '' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                              ...
                              <div class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}"
                                  ...
                                  <div class="table-responsive payment-gateway-section">
                                      ...
                                      <table class="table table-checkout">
                                          ...
                                          <tr>
                                            <td><strong>{{ __('Product Description') }}</strong></td> 
                                            <td class="text-right">{{ $info->product_description ?? '' }}</td>
                                          </tr>
                                          <tr>
                                              <td><strong>{{ __('Amount') }}</strong></td>
                                              <td class="text-right">{{ $requestData->amount }}</td>
                                              <td><strong>{{ __('Quantity') }}</strong></td>
                                              <td class="text-right">{{ $requestData->quantity }}</td>
                                          </tr>
                                          <tr>
                                              <td><strong>{{ __('Tax (%)') }}</strong></td>
                                              <td class="text-right">{{ $requestData->tax }}</td>
                                              <td><strong>{{ __('Discount (%)') }}</strong></td>
                                              <td class="text-right">{{ $requestData->discount }}</td>
                                          </tr>
                                          <tr>
                                          ...
                                          <tr>
                                            ...
                                            <td class="text-right">
                                                {{ $final_amount = ($requestData->amount * $requestData->quantity * $gateways->rate + ($requestData->amount * $requestData->quantity * $requestData->tax / 100) - ($requestData->amount * $requestData->quantity * $requestData->discount / 100) + $gateways->charge) }}
                                                {{ strtoupper($gateways->currency_name) }}
                                            </td>
                                          </tr>
                                          <tr>
                                            <td><strong>{{ __('Notes') }}</strong></td> 
                                            <td class="text-right">{{ $info->notes ?? '' }}</td>
                                          </tr> 
                                      </table>
                                    </div>
                                    <form action="{{ route('checkout.payment.view') }}" method="post" class="form" enctype="multipart/form-data">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <input type="hidden" name="access_code" value="{{ $gateway->data}}">
